...
interface FileUploadFullScreenModalProps {
  open: boolean;
  onClose: () => void;
}

export default function FileUploadFullScreenModal({
  open,
  onClose,
}: FileUploadFullScreenModalProps) {
  const {
    isAiLoading,
    reviewActionItems,
    reviewSummary,
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await api.post<AiSummaryWithTasksResponse>('/uploads', formData, {
        headers: {
      ...
      if (response.data) {
        setAiInput(response.data.summary);
        await handleAiCreate(); // This will set reviewActionItems
        setShowReview(true);
      } else {
        throw new Error('No AI response returned from upload.');
      }
    } catch (error) {
              ...
              <div className="border-2 border-dashed border-zinc-300 dark:border-zinc-700 rounded-lg p-6 text-center">
                ...
                >
                  ...
                  <span className="text-xs text-zinc-500 dark:text-zinc-500">
                    Supported formats: TXT, MD, MARKDOWN, DOCX, PDF
                  </span>
                </label>
              </div>
              {uploadError && (
